...
public:
  ...
  float history_times[100] = {0.0}; // initialization
  bool * rewind;
  float sleep_for_rewind;
  ...
  rclcpp::Subscription<std_msgs::msg::String>::SharedPtr dir_subscription_;
  ...
  Wall_Follower(const char *xml_tree) : rclcpp::Node("Wall_Follower")
  {
    ...
    for (auto &node : this->tree.nodes)
    {
      ...
      {
        rewind->init(&(this->follow_right), this->publisher_, &(this->twist_msg), this->dist_th, this->history_actions, this->history_times, this->rewind);
      }
    }
    ...
    this->dir_subscription_ = this->create_subscription<std_msgs::msg::String>(
        "/direction",
        rclcpp::QoS(rclcpp::SystemDefaultsQoS()),
...
private:
  ...
  void direction_callback(const std_msgs::msg::String::SharedPtr _msg)
  {
    if (_msg->data == "RIGHT")
      this->follow_right = true;
    if (_msg->data == "RIGHT")
